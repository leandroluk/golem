...
**Current Work:** M1-M14 concluídos (ver histórico abaixo). ROADMAP.md ganhou M15-M21 (AD-034), e **M15 está DONE** (AD-036/AD-037): `internal/dialecttest` (9 grupos de subteste) + `driver/postgres/conformance_integration_test.go` como primeiro caller, verificado de verdade com `task test-integration` contra Postgres real. A suíte achou e corrigiu 3 bugs reais que nenhum teste anterior (unit ou integration) exercitava: tipos nativos do pgx (`pgtype.Numeric`/`pgtype.Time`/`[16]byte`/objeto-ou-array JSON) vazando pra fora do `Dialect` e quebrando leitura simples de coluna DECIMAL/TIME/UUID/JSON; e `repository.assignFieldValue` não sabia escanear valor não-NULL pra dentro de campo `*X` nullable. Coverage 100% mantido (só `Scan`'s branch de erro do `Float64Value`, essencialmente inalcançável, fica de fora — mesmo padrão de exceção documentada já aplicado antes). M16-M21 continuam não iniciados. `v0.1.0` foi cortado como primeira release (AD-033, versionamento preso em major `0` pra sempre). Ver "Deferred Ideas"/"Future Considerations" pra outros candidatos (backfill de coverage legado, isolation level configurável, CI não roda integration tests ainda, etc).
