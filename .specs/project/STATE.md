...
**Current Work:** M1-M14 concluídos (ver histórico abaixo). ROADMAP.md ganhou M15-M21 (AD-034): M15 é uma suíte de conformidade cross-dialect (extraída dos testes de integração do `driver/postgres`) que todo adapter futuro deve passar, seguida de um adapter por banco listado em INSIGHT.md — MySQL/MariaDB (M16), SQLite (M17), MSSQL (M18), Oracle (M19), IBM Db2 (M20), Snowflake/OLAP (M21, escopo reduzido — sem locking, sem CHECK). Nenhum desses milestones foi iniciado ainda; só a ordem e o escopo de cada um foram definidos. `v0.1.0` foi cortado como primeira release (AD-033, versionamento preso em major `0` pra sempre). Ver "Deferred Ideas"/"Future Considerations" pra outros candidatos (backfill de coverage legado, isolation level configurável, etc).
