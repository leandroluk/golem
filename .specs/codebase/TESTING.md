...
**Test execution tooling:** a root `Taskfile.yml` wraps every command below, so the user doesn't need to remember raw `go`/`docker` invocations — mirrors `package.json` scripts.
...
test-integration:   docker compose -f .docker/docker-compose.test.yml up -d --wait
                    go test -tags=integration ./...      # defer ensures docker compose down runs in Taskfile
