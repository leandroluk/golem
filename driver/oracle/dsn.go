...
// resolveDSN combines o.DSN and the discrete fields into one final Oracle
// connection string. Confirmed via a real Oracle probe (see design.md):
// go-ora's DSN format is "oracle://user:pass@host:port/service_name" —
// net/url-parseable, with the service name in the URL path (like Postgres's
// shape), not a query param (unlike MSSQL's "?database=").
//
...
func resolveDSN(o *Options) (string, error) {
	hasDiscrete := o.Host != "" || o.Port != 0 || o.User != "" ||
		o.Password != "" || o.ServiceName != ""

	if o.DSN == "" && !hasDiscrete {
		return "", fmt.Errorf("oracle: no DSN or connection fields provided")
	}
	...
	if err != nil {
		return "", fmt.Errorf("oracle: malformed DSN: %w", err)
	}
	...
	if o.ServiceName != "" {
		u.Path = "/" + strings.TrimPrefix(o.ServiceName, "/")
	}
