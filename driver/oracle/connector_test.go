...
func TestConnector_Connect_Success(t *testing.T) {
	...
	opts := &Options{Logging: true, DSN: "oracle://user:pass@localhost:1521/FREEPDB1"}
...
func TestConnector_Connect_ErrorNewSQLDB(t *testing.T) {
	...
	opts := &Options{DSN: "oracle://user:pass@localhost:1521/FREEPDB1"}
	c := &connector{opts: opts}
	_, err := c.Connect()
	if err == nil || err.Error() != "oracle: connect: new db error" {
...
func TestConnector_Connect_ErrorPing(t *testing.T) {
	...
	c := &connector{opts: opts}
	_, err = c.Connect()
	if err == nil || err.Error() != "oracle: connect: ping error" {
