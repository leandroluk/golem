...
const defaultTestDSN = "oracle://system:Golem_Test_2026@localhost:51521/FREEPDB1"

// testDSN returns the DSN to use for integration tests, honoring
// GOLEM_ORACLE_TEST_DSN when set (matching the Taskfile.yml's
...
func unreachableHostDSN(base string) string {
	return strings.Replace(base, ":51521", ":59998", 1)
}

// badCredentialsDSN swaps in a wrong password, keeping host/port/service intact.
func badCredentialsDSN(base string) string {
	return strings.Replace(base, "system:Golem_Test_2026@", "system:WrongPassword123@", 1)
}
