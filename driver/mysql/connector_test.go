...
func TestConnector_Connect_Success(t *testing.T) {
	...
	opts := &Options{Logging: true, DSN: "user:pass@tcp(localhost:3306)/db"}
...
func TestConnector_Connect_ErrorNewSQLDB(t *testing.T) {
	...
	opts := &Options{DSN: "user:pass@tcp(localhost:3306)/db"}
	c := &connector{opts: opts}
	_, err := c.Connect()
	if err == nil || err.Error() != "mysql: connect: new db error" {
...
func TestConnector_Connect_ErrorPing(t *testing.T) {
	...
	c := &connector{opts: opts}
	_, err = c.Connect()
	if err == nil || err.Error() != "mysql: connect: ping error" {
