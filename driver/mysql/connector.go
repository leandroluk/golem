...
// connector is the mysql.Connector implementation returned by New. It
...
// Close releases the pool, returning *sql.DB.Close()'s error (unlike
// pgxpool.Pool.Close(), it can fail). Safe to call even if Connect never
// succeeded (c.db would be nil).
