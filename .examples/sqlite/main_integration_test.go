...
func TestMain(m *testing.M) {
	dsn := resolveDSN()
	ds, err := golem.NewDataSource(sqlite.New(func(o *sqlite.Options) {
		o.Path = dsn
...
func TestBlogExample_FullFlow(t *testing.T) {
	dsn := resolveDSN()
	dataSource, err := golem.NewDataSource(sqlite.New(func(o *sqlite.Options) { o.Path = dsn }), golem.DataSourceName(t.Name()))
	if err != nil {
...
// Note: SQLite doesn't truly block concurrently on a single row SELECT FOR UPDATE in the
// same way Postgres/MySQL do (it locks the whole db during write, but read locks behave differently).
// We'll skip pessimistic lock concurrency tests for SQLite in this example suite.
...
func TestBlogExample_TypedErrors(t *testing.T) {
	...
	// SQLite with PRAGMA foreign_keys = ON will return a constraint violation
...
func TestBlogExample_RawExec(t *testing.T) {
	...
	result, err := dataSource.Exec(ctx, "UPDATE users SET name = ? WHERE id = ?", "Raw Exec User Updated", u.ID)
	if err != nil {
	...
	checkRes, err := dataSource.Exec(ctx, "SELECT name FROM users WHERE id = ?", u.ID)
	if err != nil {
	...
	users, err := userRepo.Exec(ctx, "SELECT * FROM users WHERE email = ?", "raw.exec@email.com")
	if err != nil {
