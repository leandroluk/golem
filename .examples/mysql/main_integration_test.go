...
func TestMain(m *testing.M) {
	...
	ds, err := golem.NewDataSource(mysql.New(func(o *mysql.Options) {
		o.DSN = dsn
...
// against the real Dockerized test MySQL
func TestBlogExample_FullFlow(t *testing.T) {
	dsn := resolveDSN()

	dataSource, err := golem.NewDataSource(mysql.New(func(o *mysql.Options) {
		o.DSN = dsn
...
func TestBlogExample_RawExec(t *testing.T) {
	...
	// mysql doesn't support RETURNING, query to check
	...
	for checkRes.Next() {
		...
		scannedNames = append(scannedNames, string(row["name"].([]byte)))
	}
